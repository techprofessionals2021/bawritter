<form role="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('patch_settings_paystack') }}" method="post" autocomplete="off" >
   ...
   <div class="form-group">
      <label class="sky">Secret Key <span class="required">*</span></label>
      <input type="text" class="text-grey form-control {{ showErrorClass($errors,'secret_key') }}" name="secret_key" value="{{ old('secret_key', optional($rec->keys)->secret_key) }}">
