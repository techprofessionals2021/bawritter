<form role="form" class="form-horizontal" enctype="multipart/form-data" action="{{ route('patch_settings_payu') }}" method="post" autocomplete="off" >
   ...
   <div class="form-group">
      <label class="sky">Merchant Key <span class="required">*</span></label>
      <input type="text" class=" text-grey form-control {{ showErrorClass($errors,'merchant_key') }}" name="merchant_key" value="{{ old('merchant_key', optional($rec->keys)->merchant_key) }}">
      <div class="invalid-feedback">{{ showError($errors,'merchant_key') }}</div>
   </div>
   <div class="form-group">
      <label class="sky">Merchant Salt <span class="required">*</span></label>
      <input type="text" class="text-grey form-control {{ showErrorClass($errors,'merchant_salt') }}" name="merchant_salt" value="{{ old('merchant_salt', optional($rec->keys)->merchant_salt) }}">
